...
语言： [English](api-reference.md) | **简体中文**
...
签名中出现的外部类型来自 `Microsoft.Xna.Framework`（`GameTime` / `Color` / `Vector2` / `Rectangle`）、
`Microsoft.Xna.Framework.Input`（`Keys`）与 `Microsoft.Xna.Framework.Graphics`（`Texture2D`）。
...
可选能力：暴露模组的主要功能开关。它是配置层的轻量切换，不会加载、卸载模组，也不会重新安装
patch，因此适合在进入世界后由设置中心操作。模组应把它绑定到自己的配置，并在 setter 中保存配置、
让实际功能读取同一状态。
...
这不是模组主启用开关的替代品：主启用开关仍由 `IModRegistry.TrySetEnabled` 管理，进入世界后按会话
规则锁定。设置中心通过 [`IModInfo.FeatureToggle`](#imodregistry--imodinfo) 暴露该能力；模组未加载或
当前会话不允许操作时，该属性为 `null`。
...
TIMF 用**两根正交的轴**描述一个 mod。详细设计理由见 [侧别与协议模型](./side-model.zh-CN.md)。
...
- `IsDeferredAuthority`：纯权威 mod 在会话授予权威前无事可做，因此加载器**推迟其程序集加载**到激活时，并在停用时卸载。带客户端半边的 mod 不受这条“纯权威延迟”规则影响；它仍按 `LoadBeforeWorld` 或默认世界阶段决定具体加载时机。
...
public bool LoadBeforeWorld { get; set; }   // 注入后、进入世界前加载；内容/基础设施常用
...
`LoadBeforeWorld` 默认为 `false`。未声明预世界加载的模组会在进入世界时加载，并在回到主菜单时卸载；内容
模组会自动提升到预世界阶段，预世界模组的硬依赖也会被一并提升。纯 `Authority` 模组仍要等到权威路径
激活时才会实例化。
...
跨 mod 服务解析总线。普通模组经 `IModContext.Services` 解析服务；框架库（例如 `TIMF.UI`、`TIMF.Pinyin`）
会在这里提供共享能力。发布自定义服务必须改走绑定调用方程序集的
...
public interface IModInfo
{
    ...
    bool LoadsBeforeWorld { get; }       // 预世界加载标记（显式、内容或依赖提升）
    IModFeatureToggle FeatureToggle { get; } // 可操作时非 null；世界内功能开关
...
Core 注册的只读共享服务。它从游戏实际加载的 `TextureAssets` 数组中取出已经解码的 XNA 纹理，避免
模组自行引用游戏内嵌的 ReLogic 程序集或重复编写反射适配。仅在客户端绘制路径使用；资源尚未加载、字段
不存在或运行在专用服时，返回值可能为 `null`。
...
`arrayFieldName` 对应 `Terraria.GameContent.TextureAssets` 的静态数组字段名，例如 `Npc`、
`NpcHeadBoss`、`Projectile`；调用方仍需处理纹理为 `null` 的情况。
