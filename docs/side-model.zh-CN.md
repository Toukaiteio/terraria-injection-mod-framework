...
| 行为 | 规则 |
|---|---|
| 预世界加载 | `[TimfMod(LoadBeforeWorld = true)]`、内容 mod，或前置 mod 的硬依赖 → 注入后、主菜单前加载 |
| 默认加载时机 | 未被提升为预世界加载的 mod → 进入世界时加载，回到主菜单时卸载 |
| 权威路径激活 | `Side` 含 `Authority` → 按单人/主机/专用服或握手结果激活权威半边 |
| 纯权威 mod 是否延迟加载 | `Side == Authority`（无 `Client` 位）→ 即使已发现也等到权威路径激活时加载，停用时卸载 |
...
**`IModFeatureToggle` 不等于模组主开关。** 主开关由 `IModRegistry.TrySetEnabled` 管理，进入世界后对 `Authority` / `Both` 会锁定；功能开关是模组自己实现的、可在世界内安全切换的配置状态，不会调用 `Load` / `Unload`。只有模组已加载且当前会话允许操作时，`IModInfo.FeatureToggle` 才会非 null。

**预世界加载要克制使用。** 它适合内容注册、共享服务发布和必须在主菜单可用的基础设施；普通 UI、地图覆盖层和玩法逻辑保持默认世界阶段加载，可以让菜单切换不承担不必要的初始化成本。
