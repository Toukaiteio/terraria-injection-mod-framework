...
语言： [English](writing-a-mod.md) | **简体中文**

从零到能跑的最短路径。API 细节见 [API 参考](./api-reference.zh-CN.md)，侧别概念见 [侧别与协议模型](./side-model.zh-CN.md)。
...
如果模组需要在进入世界后临时关闭主要功能，实现 `IModFeatureToggle`，并把属性绑定到模组自己的配置：

```csharp
public sealed class MyMod : IClientMod, IModSettings, IModFeatureToggle
{
    // _config 与 SaveConfig() 代表你自己的配置层；这里只展示开关契约。
...
`FeatureEnabled` 只应改变轻量配置状态；实际绘制、钩子或玩法逻辑要在每帧/每次回调读取它。
它不会调用 `Load`、`Unload` 或重新打补丁。F9 设置中心会通过 `IModInfo.FeatureToggle` 显示它，
而模组主启用开关仍然只能在主菜单修改。`SaveConfig()` 代表你自己的配置保存函数，具体实现应使用
`context.Storage`。
...
| 提供设置页 | `IModSettings` |
| 在世界内切换主要功能 | `IModFeatureToggle`（配置层开关，不触发加载/卸载） |
| 改天气 | `IAuthorityServices.Weather` |
| 复用原版纹理 | `context.Services.GetService<IVanillaTextures>()` |
...
`build.ps1` 还会构建并部署随框架提供的 `TIMF.UI` 与 `TIMF.Pinyin` 库模组。`CreativeMode` 的中文/拼音
搜索依赖后者；即使库缺失，示例也会回退到普通名称和 ID 搜索。
...
- [ ] 需要在世界内临时停用功能时实现了 `IModFeatureToggle`，而不是尝试切换主启用开关
- [ ] 只有必须在进世界前准备好的内容/服务才设置 `LoadBeforeWorld = true` 吗
